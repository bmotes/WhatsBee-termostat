...
boolean configTopic (String topic, String payload) {
  ...
  if (topic.startsWith(deviceTopic, 0)) { //es un parámetro de configuración porque está por debajo del devicetopic
    if (topic.startsWith(deviceTopic + String("EPT/"), 0)) { //Es un endPoinTopic
          Serial.println ("Es un EndPointTopic");
      ...
      if (shortTopic == String("setsNodeName")) {
        ...
        sendNodeLog(PSTR("COMMAND: MQTT reset requested"));
        ESP.reset();
        return true;
      }
      else if (shortTopic == String("cPubConfig")) {
        sendNodeLog (PSTR("COMMAND: Config requested"));
        sendConfigInfo();
        return true;
      }
      else if (shortTopic == String("cPortal")) {
        sendNodeLog (PSTR("COMMAND: Portal requested"));
...
//***********************END POINTS************************************
      ...
      return true;
        ...
        sprintf_P(log, PSTR("MQTT: config received \"%s\" = \"%s\""), topic.c_str(), payload.c_str());
        addLog(LOG_LEVEL_DEBUG, log);
        return true;
    }
  } //Era un configTopic
